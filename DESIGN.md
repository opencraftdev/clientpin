...
- **Community (landing):** a social-proof wall on the paper canvas (`--color-bg`, `border-t`), deliberately light, not a dark band (a dark band would fight the light editorial theme). Centered header: coral `COMMUNITY` eyebrow, display `Join the community`, one `--color-ink-dim` line, then a single primary action, a coral `Join us on Discord` button carrying the `.shadow-edge` signature (same gesture as the hero CTA). Below it a **masonry testimonial wall** (`columns-2/3`, `break-inside-avoid`) so posts keep their natural height instead of a rigid same-size grid. Each post is an editorial card (`--color-surface`, hairline, near-sharp corners): a neutral initial-avatar (`rounded-full`, `--color-line` ring, no rainbow tints), display name, mono `@handle` in `--color-ink-mute`, a muted X glyph in the corner, then the quote in `--color-ink-dim`. Restrained: coral is spent only on the eyebrow and the Discord button; the wall itself stays ink-on-paper. This testimonial pattern is genuinely the right affordance, so the identical-card-grid ban does not apply, the masonry plus varied copy keeps it from reading as a template.
